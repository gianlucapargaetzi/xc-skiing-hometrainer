...
class IntervallParser():
    ...
    def evaluate(self):
        ...
        for (block_name, block_data), (next_block_name, next_block_data) in itertools.zip_longest(
            self._data.items(), itertools.islice(self._data.items(), 1, None), fillvalue=(None, None)):
            ...
            if block_data['type'] == 'transition':
                ...
                if transition_type == 'None':
                    ...
                    x, y = createTorqueCurve(ease_in=0.49, ease_out=0.49, xkrit=0.5, y0=y0, y1=y1,
                                             len=transition_duration, relative=True)
                    x = x + time
                x_all.append(x)
                y_all.append(y)
            ...
            if block_data['type'] == 'interval_block':
                ...
                for i in range(int(block_data['block_amount'])):
                    if i and do_transition:
                        if transition_type == 'None':
                            x = np.linspace(time, time + transition_duration - 1, transition_duration)
                            y = np.linspace(int(block_data['off_intensity']),
                                            int(block_data['on_intensity']), transition_duration)
                        elif transition_type == 'ease':
                            x, y = createTorqueCurve(ease_in=0.49, ease_out=0.49, xkrit=0.5,
                                                     y0=int(block_data['off_intensity']),
                                                     y1=int(block_data['on_intensity']),
                                                     len=transition_duration, relative=True)
                            x = x + time
                        x_block.append(x)
                        y_block.append(y)
                    ...
                    if do_transition:
                        if transition_type == 'None':
                            x = np.linspace(time, time + transition_duration - 1, transition_duration)
                            y = np.linspace(int(block_data['on_intensity']),
                                            int(block_data['off_intensity']), transition_duration)
                        elif transition_type == 'ease':
                            x, y = createTorqueCurve(ease_in=0.45, ease_out=0.45, xkrit=0.5,
                                                     y0=int(block_data['on_intensity']),
                                                     y1=int(block_data['off_intensity']),
                                                     len=transition_duration, relative=True)
                            x = x + time
                        x_block.append(x)
                        y_block.append(y)
...
class IntervallIntensityController(BackendNode, IntensityControllerInterface):
    def __init__(self):
        ...
        self.active = True  # Webinterface-Stopflag
        ...
        self._flask_requests.append(("/stop", self._stop, ['POST']))  # Stop-Button Route
    ...
    def publish(self):
        return Backend().publish("interval_intensity", {
    ...
    def _get_list(self):
        ...
        try:
            return jsonify(ret_json(True, filelist=list_files(os.path.expanduser(INTERVAL_PATH),
                                                              INTERVAL_FILENAME_ENDING)))
        except Exception as e:
    ...
    def _load_file(self):
        ...
        if not self._interval.isValid():
            Logger().error(f"{self}: Error occurred while parsing interval file")
    ...
    def stop(self):
        if self._controller_state in (ControllerState.RUNNING, ControllerState.PAUSED):
    ...
    def getIntensity(self) -> float:
        if not self.active:
            return 0  # Optional: Sofort 0% Leistung nach STOP
